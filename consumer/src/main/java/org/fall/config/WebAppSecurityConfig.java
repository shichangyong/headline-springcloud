...
import org.fall.handler.CustomAuthenticationFailureHandler;
import org.fall.handler.CustomAuthenticationSuccessHandler;
...
//启动web环境下权限控制功能
@EnableWebSecurity
//启用全局方法权限控制，并且设置prePostEnabled = true。保证@PreAuthorize等注解生效。
...
//                .withUser("17623138621")        // 设置用户名
//                .password(new BCryptPasswordEncoder().encode("2000119520"))              // 设置密码
//                .roles("ADMIN");                 // 设置权限
    ...
    protected void configure(HttpSecurity security) throws Exception {
        String[] permitUrls = {"/get/**","/security/**"};
        security
                ...
                .formLogin().successForwardUrl("/login/success")                                   // 开启表单登录功能
                .usernameParameter("login-user")                // 设置表单中对应用户名的标签的name属性名
                .passwordParameter("login-pwd")                 // 设置表单中对应密码的标签的name属性名
                .loginProcessingUrl("/security/do/login")     // 设置登录请求的提交地址
//                .failureForwardUrl("/login/fail")               // 设置登陆失败后前往的地址
                .successHandler(customAuthenticationSuccessHandler)
                .failureHandler(customAuthenticationFailureHandler)
//                .defaultSuccessUrl("/login/success")            // 设置登陆成功后前往的地址
                .and()
                .logout()                                       // 开启退出登录功能
                .logoutUrl("/security/do/logout")          // 设置退出登录的url
                .logoutSuccessUrl("/login/anonymous")    // 设置退出成功后前往的页面
